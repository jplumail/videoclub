...
export class BucketManager {
    ...
    public static async getMovies(videoId: string): Promise<MoviesData> {
        ...
        const moviesFiles = files.filter(file => file.name.endsWith('movies.json'));
...
export class ConfigurationManager {
    ...
    private static async getPosterSize() {
        return (await this.getConfigurationDetails()).images.poster_sizes.filter(size => size === 'w185')[0];
    }
